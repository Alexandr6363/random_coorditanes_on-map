...
class Coordinate:
        ...
        def __str__(self):
            return f"https://www.google.com/maps/place/{self.parallels_degrees}%C2%B0{self.parallels_minutes}'{self.parallels_seconds}%22{self.side_SN}+" \
                   f"{self.meridian_degrees}%C2%B0{self.meridian_minutes}'{self.meridian_seconds}%22{self.side_EW}/"

        def get_meridian(self):
            return f"{self.side_EW} {self.meridian_degrees} {self.meridian_minutes} {self.meridian_seconds}"

        def get_parallel(self):
            return f"{self.side_SN} {self.parallels_degrees} {self.parallels_minutes} {self.parallels_seconds}"
